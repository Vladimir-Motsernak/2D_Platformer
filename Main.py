...
class Player(pygame.sprite.Sprite):
    ...
    def jump(self):
        # Only allow jump if vertical velocity is reasonable (not falling too fast)
        if self.y_vel < 9.81:
...
def select_character(window):
    """Simple character selection screen with placeholder slots"""
    # Character list - only "snowl" asset exists for now, others will use placeholders
    characters = ["snowl", "slot2", "slot3", "slot4"]
    idx = 0
    font = pygame.font.Font(None, 36)
    selecting = True
    ...
    while selecting:
        ...
        slot_h = 160
        gap = 40
        total_w = len(characters) * slot_w + (len(characters) - 1) * gap
        ...
        for i, ch in enumerate(characters):
            ...
            # Try to draw character idle sprite if available
            sprites = load_sprite_sheets("MainCharacters", ch, 32, 32, True)
            ...
            label = font.render(ch if ch != "slot2" and ch != "slot3" and ch != "slot4" else "Empty", True, (255, 255, 255))
